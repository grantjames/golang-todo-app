...
func TestPUTTodos(t *testing.T) {
	...
	t.Run("it updates a todo on PUT", func(t *testing.T) {
		req, _ := http.NewRequest(http.MethodPut, "/v1/todos/stub-id", bytes.NewBuffer([]byte(`{"status":"Completed"}`)))
		response := httptest.NewRecorder()
...
func newPostTodoRequest() *http.Request {
	req, _ := http.NewRequest(http.MethodPost, "/v1/todos/", bytes.NewBuffer([]byte(`{"description":"New todo"}`)))
	return req
}
